...
export function AgentAudioVisualizerBar({
  ...
  const bands = useMemo(
    () => {
      // If we have an audio track, show volume reactivity even in listening state
      // but maybe dampened if not explicitly 'speaking'
      const isActuallySpeaking = state === 'speaking';
      const baseline = isActuallySpeaking ? 0.2 : 0.05; // Resting height
      
      return volumeBands.map(v => Math.max(baseline, v * (isActuallySpeaking ? 1.5 : 0.5)));
    },
    [state, volumeBands],
  );
