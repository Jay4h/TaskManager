...
export class ProjectsService {
     ...
     */
    async updateProject(projectId: string, data: Partial<CreateProjectRequest>, adminId: string): Promise<any> {
        const isAdmin = await this.isUserAdmin(adminId);
        if (!isAdmin) {
        ...
        if (data.projectDescription) {
            ...
            updateData.projectDescription = data.projectDescription.trim();
        }

        if (data.assignedUsers) {
            await this.validateAssignedUsers(data.assignedUsers);
            updateData.assignedUsers = data.assignedUsers.map((id) => new mongoose.Types.ObjectId(id));
        }

        const updatedProject = await this.projectModel.update(projectId, updateData);
     ...
     */
    async deleteProject(projectId: string, adminId: string): Promise<any> {
        const isAdmin = await this.isUserAdmin(adminId);
        if (!isAdmin) {
     ...
     */
    async getProjectsForUser(targetUserId: string, adminId: string): Promise<any> {
        const isAdmin = await this.isUserAdmin(adminId);
        if (!isAdmin) {
            throw new Error("Only admins can view projects for other users");
        }
        ...
        const projects = await this.projectModel.findByAssignedUser(targetUserId);
